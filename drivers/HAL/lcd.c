...
void lcdInit(uint8_t ui8FunctionSetOptions, uint8_t ui8EntryModeSetOptions, uint8_t ui8DisplayOptions) {
	...
	SysCtlGPIOAHBEnable(LCD_GPIO_DATA_PERIPH);
	SysCtlGPIOAHBEnable(LCD_GPIO_CTL_PERIPH);

	SysCtlPeripheralEnable(LCD_GPIO_DATA_PERIPH);
	SysCtlPeripheralEnable(LCD_GPIO_CTL_PERIPH);
	
   while(!SysCtlPeripheralReady(LCD_GPIO_DATA_PERIPH)) { /* Nothing */ }
	 while(!SysCtlPeripheralReady(LCD_GPIO_CTL_PERIPH)) { /* Nothing */ }
